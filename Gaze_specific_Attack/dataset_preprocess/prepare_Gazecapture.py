...
parser.add_argument('--dataset_path', default='/datasets/external/gazecapture', help="Path to extracted files. It "
                                                                                     "should have folders called "
                                                                                     "'%%05d' in it.")
parser.add_argument('--output_path', default='./data/gazecapture',
...
def main():
    ...
    for i, recording in enumerate(recordings):
        ...
        facePath = preparePath(os.path.join(recDirOut))
    ...
    reference = sio.loadmat('/projects/tang/GMI-Attack/GMI-Attack/Celeba/data/reference_metadata.mat', struct_as_record=False)
    ...
    if nMissing > 0:
        print(
            'There are %d frames missing in the new dataset. This may affect the results. Check the log to see which '
            'files are missing.' % nMissing)
    else:
        print('There are no missing files.')
    if nExtra > 0:
        print(
            'There are %d extra frames in the new dataset. This is generally ok as they were marked for validation '
            'split only.' % nExtra)
    else:
