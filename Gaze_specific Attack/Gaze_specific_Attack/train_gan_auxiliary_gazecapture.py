...
'''
    This file is used to train attack model(GAN) with auxiliary(blured images) based on GazeCapture Dateset.
'''

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
config = get_default_config()
config.merge_from_file('./configs/gazecapture.yaml')
config.freeze()
# public list does not matter
train_dataset, val_dataset = create_dataset(config, True, [range(15)], False, True)
...
save_img_dir = "result/imgs_gan_gazecapture"
save_model_dir = "result/models_gan_gazecapture"
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        start = time.time()
        for i, (images, blurred, gazes) in enumerate(train_loader):
            step += 1
        ...
        if (epoch + 1) % 1 == 0:
            ...
            i = 0
            for x in index:
                img, blur_img, tmp = train_dataset.__getitem__(x)
                actual[i] = img
            ...
            save_tensor_images(images.detach(), os.path.join(save_img_dir, f"result_gazecapture_auxiliary_{epoch}.png"),
                               nrow=save_size)

        torch.save({'state_dict': Net.state_dict()}, os.path.join(save_model_dir, "gazecapture_G_auxiliary.tar"))
        torch.save({'state_dict': DG.state_dict()}, os.path.join(save_model_dir, "gazecapture_D_auxiliary.tar"))
