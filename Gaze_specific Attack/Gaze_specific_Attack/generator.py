...
''''
Generator input-dim 100 noise output [batchsize,3,64,64]
GeneratorMNIST inputdim 100 noise output [batchsize,3,28,28]

CompletionNetwork recover the images input channel = 4 outputchannel= 3 size don't change.
CompletionNetwork is used for the task of Image Inpainting.
This model contains a series of convolutional, deconvolutional, and batch normalization layers that process 
the input image step-by-step to ultimately output the restored image.

IdentityGenerator The first half is a fully-connected neural network (MLP) and the second half is a section consisting 
of multiple inverse convolutional layers and batch normalization layers. Input 100 Noise Output (batch_size, 128, 4, 4)

InversionNet is an image inversion model consisting of a ContextNetwork and an IdentityGenerator network.
Its goal is to generate a complete image by combining the input x (which contains some image information) 
and the hidden variable z (noise). The size of the output image is (batch_size, 3, h, w)
'''
