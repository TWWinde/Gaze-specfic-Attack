...
config.merge_from_file('/projects/tang/GMI-Attack/GMI-Attack/Celeba/configs/lpw.yaml')
config.freeze()
train_dataset, val_dataset = create_dataset(config, True, [10, 11, 12, 13, 14], False, True, True)
...
save_img_dir = "result/imgs_gan_lpw"
save_model_dir = "result/models_gan_lpw"
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        start = time.time()
        for i, (images, blurred, pupil_local) in enumerate(train_loader):
            step += 1
        ...
        if (epoch + 1) % 1 == 0:
            ...
            save_tensor_images(images.detach(), os.path.join(save_img_dir, "result_lpw_auxiliary_{}.png".format(epoch)),
                               nrow=save_size)

        torch.save({'state_dict': Net.state_dict()}, os.path.join(save_model_dir, "lpw_G_auxiliary.tar"))
        torch.save({'state_dict': DG.state_dict()}, os.path.join(save_model_dir, "lpw_D_auxiliary.tar"))
